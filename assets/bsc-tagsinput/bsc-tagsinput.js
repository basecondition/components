...
/**
 * BscTagify (v 2.6.4)- tags input component
 * By Yair Even-Or (2016)
 * Don't sell this code. (c)
 * https://github.com/yairEO/bsctagify
 */
;(function(root, factory) {
    if (typeof define === 'function' && define.amd) {
        ...
        root.BscTagify = factory();
    }
}(this, function() {
    "use strict";

    function BscTagify(input, settings) {
        // protection
        if (!input) {
            console.warn('BscTagify: ', 'invalid input element ', input);
            return this;
        }
    ...
    BscTagify.prototype = {
        ...
        build: function build(input) {
            var that = this,
                DOM = this.DOM,
                template = "<tags class=\"bsctagify " + (this.settings.mode ? "bsctagify--mix" : "") + " " + input.className + "\" " + (this.settings.readonly ? 'readonly' : '') + ">\n                            <div contenteditable data-placeholder=\"" + input.placeholder + "\" class=\"bsctagify__input\"></div>\n                        </tags>";
        ...
        loadOriginalValues: function loadOriginalValues() {
            ...
            if (this.settings.mode == 'mix') {
                ...
                tag && tag.classList.add('bsctagify--noAnim');
            });
        },
        ...
        events: {
            ...
            binding: function binding() {
                ...
                if (bindUnbind && !this.listeners.main) {
                    ...
                    if (this.settings.isJQueryPlugin) $(this.DOM.originalInput).on('bsctagify.removeAllTags', this.removeAllTags.bind(this));
                    ...
                    switch (e.key) {
                        case 'Backspace':
                            if (s == "" || s.charCodeAt(0) == 8203) {
                                lastTag = this.DOM.scope.querySelectorAll('tag:not(.bsctagify--hide):not([readonly])');
        ...
        input: {
            ...
            /**
             * Marks the bsctagify's input as "invalid" if the value did not pass "validateTag()"
             */
            validate: function validate() {
                var isValid = !this.input.value || this.validateTag.call(this, this.input.value);
                this.DOM.input.classList.toggle('bsctagify__input--invalid', isValid !== true);
            },
        ...
        markTagByValue: function markTagByValue(value, tagElm) {
            ...
            if (tagElm) {
                tagElm.classList.add('bsctagify--mark');
                setTimeout(function () {
                    tagElm.classList.remove('bsctagify--mark');
                }, 100);
                return tagElm;
            }
        ...
        addTags: function addTags(tagsItems, clearInput) {
            ...
            tagsItems.forEach(function (tagData) {
                ...
                if (tagValidation !== true) {
                    tagData.class = tagData.class ? tagData.class + " bsctagify--notAllowed" : "bsctagify--notAllowed";
         ...
         * @param  {Boolean}        silent          [A flag, which when turned on, does not removes any value and does not update the original input value but simply removes the tag from bsctagify]
        ...
        removeTag: function removeTag(tagElm, silent) {
            ...
            function animation() {
                ...
                tagElm.classList.add('bsctagify--hide'); // manual timeout (hack, since transitionend cannot be used because of hover)
        ...
        dropdown: {
            ...
            build: function build() {
                var className = ("bsctagify__dropdown " + this.settings.dropdown.classname).trim(),
                ...
                callbacks: {
                    ...
                    onClick: function onClick(e) {
                        ...
                        listItemElm = [e.target, e.target.parentNode].filter(function (a) {
                            return a.className.includes("bsctagify__dropdown__item");
                        })[0];
            ...
            highlightOption: function highlightOption(elm, adjustScroll) {
                if (!elm) return;
                var className = "bsctagify__dropdown__item--active"; // for IE support, which doesn't allow "forEach" on "NodeList" Objects
            ...
            createListHTML: function createListHTML(list) {
                var getItem = this.settings.dropdown.itemTemplate || function (item) {
                    return "<div class='bsctagify__dropdown__item " + (item.class ? item.class : "") + "' " + getAttributesString(item) + ">" + (item.value || item) + "</div>";
                }; // for a certain Tag element, add attributes.
...
(function ($) {
    ...
    $.fn.bsctagify = function () {
        var settings = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
        return this.each(function () {
            var $input = $(this),
                bsctagify;
            if ($input.data("bsctagify")) // don't continue if already "tagified"
                return this;
            settings.isJQueryPlugin = true;
            bsctagify = new BscTagify($input[0], settings);
            $input.data("bsctagify", bsctagify);
        });
    };
